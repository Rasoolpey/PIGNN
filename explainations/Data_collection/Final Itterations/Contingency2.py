...
# ── Y-Matrix Functions ─────────────────────────────────────────────────────
def extract_y_matrix_from_powerfactory(app):
    """Extract Y-matrix directly from PowerFactory"""
    try:
        # Method 1: Try to use PowerFactory Y-matrix calculation
        print("🔧 Extracting Y-matrix from PowerFactory...")
        
        # Get all buses, lines, and transformers
        buses = app.GetCalcRelevantObjects("*.ElmTerm")
        lines = app.GetCalcRelevantObjects("*.ElmLne")
        transformers = app.GetCalcRelevantObjects("*.ElmTr2")
        ...
        bus_to_idx = {bus.loc_name: i for i, bus in enumerate(buses)}
        
        print(f"   📊 Building Y-matrix for {num_buses} buses...")
        
        # Process lines
        for line in lines:
            try:
                if getattr(line, 'outserv', 0):  # Skip if out of service
                    continue
                ...
                if bus1 and bus2:
                    i = bus_to_idx.get(bus1.loc_name)
                    j = bus_to_idx.get(bus2.loc_name)
                    
                    if i is not None and j is not None:
                        # Get line parameters
                        R = getattr(line, 'R1', 0)  # Resistance
                        X = getattr(line, 'X1', 0)  # Reactance
                        B = getattr(line, 'B1', 0)  # Susceptance
                        
                        if R != 0 or X != 0:
                            # Series admittance
                            Y_series = 1.0 / complex(R, X)
                            
                            # Shunt admittance (half on each end)
                            Y_shunt = 1j * B / 2.0
                            
                            # Add to Y-matrix
                            Y_matrix[i, j] -= Y_series  # Off-diagonal (negative)
                            Y_matrix[j, i] -= Y_series
                            Y_matrix[i, i] += Y_series + Y_shunt  # Diagonal
                            Y_matrix[j, j] += Y_series + Y_shunt
                            
            except Exception as e:
                print(f"   ⚠️ Warning processing line {getattr(line, 'loc_name', 'Unknown')}: {e}")
        
        # Process transformers
        for trafo in transformers:
            try:
                if getattr(trafo, 'outserv', 0):  # Skip if out of service
                    continue
                ...
                if bus1 and bus2:
                    ...
                    if i is not None and j is not None:
                        ...
                        ukr = getattr(trafo, 'ukr', 0) / 100  # R in p.u.
                        uk = getattr(trafo, 'uk', 0) / 100    # X in p.u.
                        strn = getattr(trafo, 'strn', 100)    # Rating MVA
                        
                        if uk != 0:  # Avoid division by zero
                            # Convert to admittance (simplified)
                            Z_base = 1.0  # Assume per-unit system
                            Y_series = 1.0 / complex(ukr, uk)
                            
                            # Add to Y-matrix
                            Y_matrix[i, j] -= Y_series
                            Y_matrix[j, i] -= Y_series
                            Y_matrix[i, i] += Y_series
                            Y_matrix[j, j] += Y_series
                            
            except Exception as e:
                print(f"   ⚠️ Warning processing transformer {getattr(trafo, 'loc_name', 'Unknown')}: {e}")
        
        # Check if Y-matrix seems reasonable
        non_zero_elements = np.count_nonzero(Y_matrix)
        print(f"   ✅ Y-matrix built: {non_zero_elements} non-zero elements")
        
        if non_zero_elements == 0:
            print("   ⚠️ Y-matrix is empty, using fallback method")
            return load_y_matrix_fallback()
        
        return Y_matrix
        
    except Exception as e:
        print(f"   ❌ Y-matrix extraction failed: {e}")
        return load_y_matrix_fallback()
...
def calculate_post_contingency_voltage_sensitivity(app, Y_matrix, bus_data):
    """Calculate voltage sensitivity matrix for the post-contingency system"""
    try:
        print("🔍 Calculating post-contingency voltage sensitivity...")
        
        num_buses = len(bus_data)
        dV_dP = np.zeros((num_buses, num_buses))
        success_flags = np.zeros(num_buses, dtype=bool)
        
        # Get current system state (post-contingency)
        buses = app.GetCalcRelevantObjects("*.ElmTerm")
        
        # Method: Small perturbation analysis
        base_voltages = bus_data['voltage_pu'].values
        perturbation_mw = 10.0  # Small perturbation: 10 MW
        
        # Try perturbation method first (simplified)
        print("   🔄 Attempting simplified perturbation method...")
        
        for j in range(min(5, num_buses)):  # Test only first 5 buses to avoid too many errors
            try:
                # Get the bus object
                target_bus = buses[j]
                
                # Try to find existing load at this bus
                existing_loads = target_bus.GetContents("*.ElmLod")
                
                if existing_loads:
                    # Modify existing load
                    test_load = existing_loads[0]
                    original_p = getattr(test_load, 'plini', 0)
                    original_q = getattr(test_load, 'qlini', 0)
                    
                    # Apply perturbation
                    test_load.plini = original_p + perturbation_mw
                    test_load.qlini = original_q + perturbation_mw * 0.3
                    
                    # Run power flow with perturbation
                    convergence = run_power_flow(app, f"(sensitivity test bus {j+1})")
                    
                    if convergence:
                        # Collect new voltages
                        perturbed_voltages = collect_bus_voltage_array(app)
                        
                        # Calculate sensitivity: dV/dP
                        voltage_change = perturbed_voltages - base_voltages
                        dV_dP[:, j] = voltage_change / perturbation_mw
                        success_flags[j] = True
                    
                    # Restore original values
                    test_load.plini = original_p
                    test_load.qlini = original_q
                    
                    # Restore original state
                    run_power_flow(app, f"(restore after bus {j+1})")
                    
                else:
                    # No existing load, skip perturbation for this bus
                    success_flags[j] = False
                    
            except Exception as e:
                print(f"   ⚠️ Sensitivity calculation failed for bus {j+1}: {e}")
                success_flags[j] = False
        
        # Use Y-matrix approximation for all calculations (more reliable)
        print("   🔄 Using Y-matrix approximation for sensitivity...")
        dV_dP = calculate_sensitivity_from_y_matrix(Y_matrix)
        success_flags[:] = True
        
        successful_calculations = np.sum(success_flags)
        print(f"   ✅ Voltage sensitivity calculated: {successful_calculations}/{num_buses} buses successful")
        
        return dV_dP, success_flags
        
    except Exception as e:
        print(f"   ❌ Voltage sensitivity calculation failed: {e}")
        # Return approximate sensitivity
        dV_dP = calculate_sensitivity_from_y_matrix(Y_matrix)
        success_flags = np.ones(num_buses, dtype=bool)
        return dV_dP, success_flags

def calculate_sensitivity_from_y_matrix(Y_matrix):
    """Calculate approximate voltage sensitivity from Y-matrix"""
    try:
        # Method: dV/dP ≈ Real(Z_matrix) where Z = inv(Y)
        Z_matrix = np.linalg.pinv(Y_matrix)  # Pseudo-inverse for robustness
        dV_dP = np.real(Z_matrix)
        return dV_dP
    except:
        # Fallback: Identity matrix
        return np.eye(Y_matrix.shape[0])
...
def comprehensive_contingency_analysis():
    ...
    # Extract baseline Y-matrix
    print("🔧 Extracting baseline Y-matrix...")
    baseline_Y_matrix = extract_y_matrix_from_powerfactory(app)
    
    # Calculate baseline voltage sensitivity
    print("🔍 Calculating baseline voltage sensitivity...")
    baseline_dV_dP, baseline_success_flags = calculate_post_contingency_voltage_sensitivity(
    ...
    # Extract contingency Y-matrix (post-contingency network topology)
    print("🔧 Extracting post-contingency Y-matrix...")
    contingency_Y_matrix = extract_y_matrix_from_powerfactory(app)
    
    # Calculate post-contingency voltage sensitivity
    print("🔍 Calculating post-contingency voltage sensitivity...")
    contingency_dV_dP, contingency_success_flags = calculate_post_contingency_voltage_sensitivity(
    ...
    analysis_results = {
        ...
        'y_matrix_analysis': {
            'baseline_y_matrix_nonzeros': int(np.count_nonzero(baseline_Y_matrix)),
            'contingency_y_matrix_nonzeros': int(np.count_nonzero(contingency_Y_matrix)),
            'y_matrix_change_elements': int(np.count_nonzero(contingency_Y_matrix - baseline_Y_matrix)),
            'baseline_y_matrix_condition': float(np.linalg.cond(baseline_Y_matrix.real)) if baseline_Y_matrix.size > 0 else 0,
            'contingency_y_matrix_condition': float(np.linalg.cond(contingency_Y_matrix.real)) if contingency_Y_matrix.size > 0 else 0
        },
        'sensitivity_analysis': {
            ...
            'sensitivity_change_metric': float(np.linalg.norm(contingency_dV_dP - baseline_dV_dP)) if baseline_dV_dP.shape == contingency_dV_dP.shape else 0
        },
        'zone_impact': {
